...
const ReplaysPage = () => {
  ...
  useEffect(() => {
    invoke({cmd: "pageChange", page: "replays"});
  }, []);

  useEffect(() => {
    listen("replaydata", payload => {
      setReplays({ data: payload.payload.data });
    });
  }, []);
    ...
    <section className="replays-page-container">
      ...
      <div className="replays">
        {replays.data.map((replay) => {
          ...
          return <Replay data={replay} imgSize={imgSize} key={replay.id} />;
        })}
      </div>
    </section>
  );
